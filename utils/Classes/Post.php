...
class Post extends User {
    ...
    // }
        ...
        public static function getMention($mention){
        ...
        public static function getPostLinks($post){
            ...
            $post = preg_replace("/@([\w]+)/", "<a class='hash-post' href='http://localhost/twitter/$1'>$0</a>", $post);
            return $post;		
        }
        public static function hashtagAndMentionPost($post){
            $post = preg_replace("/(https?:\/\/)([\w]+.)([\w\.]+)/", "<a href='$0' target='_blink'>$0</a>", $post);
            $post = preg_replace("/#([\w]+)/", "<a class='hash-post' href='#'>$0</a>", $post);		
            $post = preg_replace("/@([\w]+)/", "<a class='hash-post' href='#'>$0</a>", $post);
            return $post;		
